...
# Configuration
# Reproducibility justification: avoid machine-specific absolute paths; resolve the repo root from this script location.
...
python -m scba.scripts.prep_jsrt \
    --data_root "${SCBA_JSRT_ROOT:?Set SCBA_JSRT_ROOT or pass --data_root explicitly}" \
    --out data/jsrt \
    --visualize \
...
python -m scba.scripts.prep_montgomery \
    --data_root "${SCBA_MONTGOMERY_ROOT:?Set SCBA_MONTGOMERY_ROOT or pass --data_root explicitly}" \
    --out data/montgomery \
    --visualize \
...
if [ -n "${SCBA_JSRT_ROOT:-}" ] && { [ -f "${SCBA_JSRT_ROOT}/images" ] || [ -d "${SCBA_JSRT_ROOT}/images" ]; }; then
...
if [ -n "${SCBA_MONTGOMERY_ROOT:-}" ] && { [ -f "${SCBA_MONTGOMERY_ROOT}/CXR_png" ] || [ -d "${SCBA_MONTGOMERY_ROOT}/CXR_png" ]; }; then
