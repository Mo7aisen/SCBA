...
from scba.data.loaders.shenzhen import get_shenzhen_dataloaders
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to write/read deterministic split CSV (keeps dataset root read-only).",
    )
    ...
    if args.data_root is None:
        ...
        if args.data == "jsrt":
            env_var = "SCBA_JSRT_ROOT"
        elif args.data == "montgomery":
            env_var = "SCBA_MONTGOMERY_ROOT"
        else:
            env_var = "SCBA_SHENZHEN_ROOT"
        args.data_root = os.environ.get(env_var)
        if not args.data_root:
            raise ValueError(
                f"--data_root is required (or set {env_var}) to run training reproducibly."
            )
    ...
    if args.data == "jsrt":
        dataloaders = get_jsrt_dataloaders(
            ...
            write_splits=bool(args.splits_path),
        )
    elif args.data == "montgomery":
        ...
        dataloaders = get_shenzhen_dataloaders(
            args.data_root,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            target_size=target_size,
