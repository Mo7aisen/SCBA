...
def simple_texture_fill(
    image: np.ndarray,
    original_mask: np.ndarray,
    edited_mask: np.ndarray,
) -> np.ndarray:
    """
    Simple texture fill for morphological border edits (OLD SCBA method).
    Uses Poisson blending to fill the region of interest.

    Args:
        image: (H, W) original image
        original_mask: (H, W) original segmentation mask
        edited_mask: (H, W) edited mask after morphological operation

    Returns:
        Edited image with filled region
    """
    # Compute ROI (symmetric difference)
    roi = np.logical_xor(original_mask > 0.5, edited_mask > 0.5).astype(np.float32)

    # Use simple inpainting in ROI
    if roi.sum() > 0:
        return inpaint_region(image, roi, method="telea", radius=5)
    else:
        return image
