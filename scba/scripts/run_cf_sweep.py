...
def parse_args():
    ...
    # Scientific justification: default to CPU to ensure the sweep is runnable
    # in reviewer environments without CUDA (and to avoid CUDA-driver coupling).
...
def load_model(ckpt_path: str, device: str):
    ...
    # Load model
    # Scientific justification: SCBA uses binary segmentation with 2 logits
    # (background vs lung). A 1-channel head changes the task formulation and
    # invalidates all counterfactual/XAI metrics.
    model = get_model(arch, in_channels=1, out_channels=2)
...
def process_border_edit(
    ...
    for method in methods:
        ...
        try:
            # Scientific justification: explanations must target the lung
            # foreground class (1) in binary segmentation.
            sal_orig = explain(image_tensor, model, method=method, target_class=1, device=device)
            sal_dilate = explain(img_dilate_tensor, model, method=method, target_class=1, device=device)
            sal_erode = explain(img_erode_tensor, model, method=method, target_class=1, device=device)
...
def process_nodule_insertion(
    ...
    for method in methods:
        try:
            ...
            # foreground class (1) in binary segmentation.
            sal_orig = explain(image_tensor, model, method=method, target_class=1, device=device)
            sal_nodule = explain(img_nodule_tensor, model, method=method, target_class=1, device=device)
            ...
            # Scientific justification: use a consistent metric key name
            # (`shift_distance`) matching `scba.metrics.cf_consistency.coa_shift`.
            results[method] = {
                "delta_am_roi": float(delta_am),
                "am_roi_original": float(am_orig),
                "am_roi_perturbed": float(am_pert),
                "shift_distance": float(shift_metrics.get("shift_distance", 0.0)),
            }
        except Exception as e:
            print(f"Warning: Failed to compute {method}: {e}")
            results[method] = {"error": str(e)}
...
def visualize_cf_results(
    ...
    # `plt.subplots` returns a 2D axes array here because rows=(n_methods+1) and
    # cols=(n_cf+1) are both >= 2; do not reshape (it breaks indexing).
...
def main():
    ...
    if args.data_root is None:
        ...
        # Require an explicit data root or an environment variable.
        env_var = "SCBA_JSRT_ROOT" if args.data == "jsrt" else "SCBA_MONTGOMERY_ROOT"
        args.data_root = os.environ.get(env_var)
        if not args.data_root:
            raise ValueError(f"--data_root is required (or set {env_var}) to run the sweep reproducibly.")
